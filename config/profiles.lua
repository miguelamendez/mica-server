-- Proposed schema-2 engine/execution/residency catalog.
--
-- This file deliberately returns data and is not loaded by the current legacy
-- mica.profile parser.  The profile loader, validator, engine adapters, and
-- observed-memory enforcement must land before these profiles are selectable.
return {
  schema = 2,
  ...
  execution_profiles = {
    ...
    ["spark-capacity-long"] = {
      ...
      artifact = {format = "mlx", quantization = "q4"},
      context = {max_input_tokens = 14336, max_output_tokens = 2048,
                 max_total_tokens = 16384},
      thinking = {enabled = true, max_reasoning_tokens = 1536,
                  min_visible_output_tokens = 512},
      batching = {max_concurrent_requests = 1, max_batch_tokens = 16384,
                  max_queued_requests = 8},
      kv_cache = {precision = "q4", scheme = "uniform", group_size = 64,
                  quantize_after_tokens = 0, max_tokens = 16384},
    },
